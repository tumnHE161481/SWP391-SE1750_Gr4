...
// Chuyển đổi chuỗi ngày thành đối tượng Date
        ...
        Date currentDeadlineDate = null;
        Date deadlineDate = null;
        try {
            currentDeadlineDate = formatter.parse(currentdeadline);
            deadlineDate = formatter.parse(deadlineStr);
        } catch (ParseException e) {
            e.printStackTrace();
        }

// Kiểm tra nếu ngày của currentdeadline lớn hơn ngày của deadlineStr
        if (currentDeadlineDate != null && deadlineDate != null && currentDeadlineDate.compareTo(deadlineDate) > 0) {
            request.setAttribute(updateMessage, "New deadline can't be before old deadline.");
            ...
            try {
                ...
                if (success) {
                    request.setAttribute(updateMessage, "Thêm thành công");
                    response.sendRedirect(request.getContextPath() + "/roomfee?id=" + roomID);
                } else {
                    request.setAttribute(updateMessage, "Thất bại");
